...
depthArray = [1:6]/utot; % unit:mm
depthArray = [0.03 depthArray];
powerWin = cell(1,length(depthArray));
powerWin{1} = [20, 40, 50, 60]; % imaging depth = 0 attenuation length
powerWin{2} = [20, 40, 50, 60, 70, 80]; % 1 att
powerWin{3} = [40, 45, 50, 55, 60, 70, 80:40:160]; % 2 att
powerWin{4} = [40, 50, 60, 65, 70, 75, 80, 100, 180]; % 3 att
powerWin{5} = [40, 60, 70, 80, 90, 100, 150, 200]; % 4 att
powerWin{6} = [40, 70, 80, 90, 100, 110, 120, 180, 240]; % 5 att
powerWin{7} = [40, 80, 100, 120, 140, 160, 200, 280]; % 5 att
maxTemp = cell(size(powerWin));
absPortion = zeros(size(depthArray));
lostPortions = zeros(4,length(depthArray));

rootDir = './1320GaussDepthScan';
