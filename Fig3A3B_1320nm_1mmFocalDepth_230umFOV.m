...
% cranial window and immersion water for 1320-nm 3-photon imaging. This
% code was used to produce Figure 3A and 3B (1320-nm part) in ref [1]. The
% imaging condition was set at 1-mm focal depth, 230-um FOV diameter. As in
% Figure 3B, two average power levels were used for simulaation, which are
% 100 and 150 mW after the objective lens (or 85 mW and 128 mW at the brain
% suface).

% This script is simply adopted from "DepthPowerScan1320nm.m" with the
% following chagnes:
% 1. The "depthArray" array has only one element of depth 1.0 mm
% 2. The "powerWin" array has two power levels of 85 mW and 128 mW.
% 3. The params.geo.FOV is set to 0.23 (unit:um)

% Tianyu Wang 04/12/2020

% 1. T. Wang, et al, "Quantitative analysis of 1300-nm three-photon calcium
% imaging in the mouse brain," Elife 9, e53205 (2020).
...
depthArray = [1.0]; % unit:mm
powerWin = cell(1,length(depthArray));
powerWin{1} = [85, 128]; % unit:mW
...
for i = 1:length(depthArray)    
    ...
    save([rootDir, '/params_1320nm_DepthPowerScan.mat'],'params','powerWin', 'maxTemp','absPortion','lostPortions');
    
    %plot temperature at different power and depth
...
save([rootDir, '/params_1320nm_Fig3B.mat'],'params','powerWin', 'maxTemp','absPortion','lostPortions');